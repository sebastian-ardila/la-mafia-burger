...
import { List, X, ForkKnife, CalendarBlank, Clock, BookOpen, EnvelopeSimple, ShoppingCart, Translate, Table } from '@phosphor-icons/react'
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-brand/95 backdrop-blur-sm border-b border-black/10">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <Table size={14} weight={hasTable ? 'fill' : 'regular'} />
              <span className="hidden sm:inline">
                {hasTable
                  ? `${i18n.language === 'en' ? 'Table' : 'Mesa'} ${tableNumber}`
                  : i18n.language === 'en' ? 'Table' : 'Mesa'
                }
              </span>
            </button>
            <button
